...
export function Dropdown({buttonText, options = [], onSelected}: DropdownProps) {
    ...
    return(
        <div tabIndex={0} className='relative w-fit' onBlur={() => setOpen(false)}>
            ...
            {
                open &&
                <Card noPadding className="absolute flex flex-col items-center p-[0.5rem] mt-[0.5rem] w-full text-center rounded-lg overflow-y-scroll dropdown-content">
                    {
                        options.map(option => (
                            <div key={option} className='p-[0.5rem] m-[0.1rem] font-bold w-full rounded-lg cursor-pointer' onMouseDown={() => itemClicked(option)}>
                                {option}
                            </div>
                        ))
                    }
